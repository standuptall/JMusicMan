...
public class FlacReader
{
    ...
    */
    ...
    public FlacReader(String filepath)
    {
        ...
        try {
            ...
            do 
            {
                ...
                if (flag == 0b00000100 || flag == 0b10000100)   //se è un vorbis comment cioè 00000100 oppure 10000100
                {
            ...
            } while (!(((flag>>7)&1)==1)); //se il bit più significativo è uguale a uno vuol dire ch enon ci sono più metadata
            stream.close();
        }
    ...
    public void writeAll()  //file esistente quindi riverso il contenuto su un file _temp
    {
        ...
        try {
            stream = new BufferedInputStream(new FileInputStream(filepath));
            streamWrite =  new BufferedOutputStream(new FileOutputStream(filepath+"_temp"));
            byte[] array = { 0, 0, 0, 0 };
            stream.read(array, 0, 4);
